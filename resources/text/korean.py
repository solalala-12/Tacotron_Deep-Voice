...
# 영문 처리되지 X
def normalize(text):
    # print('before normal :', text)
    # text = text.strip()
    # re.sub(pattern, repl, string) / string에서 pattern과 일치하는 문장을 repl로 바꾼다.
...
count_checker = "(명|시|가지|살|마리|포기|송이|수|톨|통|점|개|벌|척|채|다발|그루|자루|줄|켤레|그릇|잔|마디|상자|사람|곡|병|판|대|건)" # 대/건 추가 -sora
...
def number_to_korean(num_str, is_count=False):
    if is_count:
        ...
        # print('num_str, unit_str ',num_str, unit_str )
    else:
