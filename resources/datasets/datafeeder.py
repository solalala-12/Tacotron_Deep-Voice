...
def get_path_dict(
    ...
    for data_dir in data_dirs:
        ...
        # Data shuffle / 임의로 생성한 데이터셋엔 필수로 해줘야 함. label이 치우쳐저 있을 가능성 큼.
        if data_type == 'train':
            rng.shuffle(paths)

        if not config.skip_path_filter:
            # items = (path, n_frame, n_token)
            items = parallel_run(
            ...
            print('min_n_frame : %d max_n_frame : %d',min_n_frame,max_n_frame)
            print('\n items \n :' , items)
            
            new_items = [(path, n) for path, n, n_tokens in items \
                    if min_n_frame <= n <= max_n_frame and hparams.min_tokens<= n_tokens <=125]
            ...
            # linear spectrum frames
            # 
            new_paths = [path for path, n in new_items]
            new_n_frames = [n for path, n in new_items]

        
            # print(type(new_paths),type(new_n_frames))
            print('\n new_paths \n :' , new_paths)

            # 전체 학습 데이터 시간 파악하기 위함.
            ...
            new_paths = paths
            print('Skip_path_filter= True / new_paths' , new_paths)
