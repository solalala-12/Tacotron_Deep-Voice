...
def download_audio_with_urls(data, out_ext="wav"):
    for d in data:
        ...
        # docker ubuntu에서 실행
        # print(d.start,d.end)
        texts='ffmpeg -i {} -ss {} -to {}  {}'.format('/home/multi-speaker-tacotron/resources/datasets/park/audio/'+original_path.split('/')[-1],
        "%02d:%02d:%02d" % (0, int(d.start.split(':')[0]), int(d.start.split(':')[1])),"%02d:%02d:%02d" % (0, int(d.end.split(':')[0]), int(d.end.split(':')[1])),'/home/multi-speaker-tacotron/resources/datasets/park/audio/'+out_path.split('/')[-1])
        # print(texts)
