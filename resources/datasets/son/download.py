...
# base url/ html parser를 이용하여 news_id tag의 text를 받아옴
def get_news_ids(page_id):
    params = { #얘네가 BASE_URL뒤에 붙어서 크롤링 할 때 보는 그거 나옴
        'NJC': 'NJC300', 
        'CAID': 'NC10011174',
        'PGI': page_id,
    }
...
def download_news_video_and_content(
    ...
    encoding = 'utf-8' #unicode error 관련해서 추가

    with open(original_text_path, "w", encoding=encoding) as f:
        f.write(text)
    ...
    if not os.path.exists(audio_path):
        ...
        # error // ubuntu 환경에서 ffmpeg 설치 필요
...
if __name__ == '__main__':
    ...
    exceptions = ["NB10830162","NB10821986"]
