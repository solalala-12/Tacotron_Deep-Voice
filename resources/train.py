...
** 수정사항
0. 코드 클리닝 (특정 데이터 셋에 대한 if문 처리 삭제)
1. 
'''
...
def save_and_plot_fn(args, log_dir, step, loss, prefix):
    ...
    if 'korean_cleaners' in [x.strip() for x in hparams.cleaners.split(',')]:
        log('Training korean : Use jamo') #여기서 에러나는 듯,,, 참고 https://github.com/carpedm20/multi-speaker-tacotron-tensorflow/issues/10
        plot.plot_alignment(
...
def train(log_dir, config):
    ...
    # tf.train.Coordinator / 멀티스레드 컨트롤
    coord = tf.train.Coordinator()
    # tf.variable_scope /네트워크 정의 / 한뭉탱이처리
    with tf.variable_scope('datafeeder') as scope:
        # 조건에 맞는 데이터 load (min_tokens 등)
        train_feeder = DataFeeder(
                # data_dirs datasets/son/data ,batches per group=10
                coord, data_dirs, hparams, config, 5,
    ...
    with tf.Session() as sess:
        ...
        # 쓰레드가 중지되면 True를 반환
            while not coord.should_stop():
                ...
                if step % config.checkpoint_interval == 0: #체크포인트 마다 기록하는 부분
                    log('Saving checkpoint to: %s-%d' % (checkpoint_path, step))
                    saver.save(sess, checkpoint_path, global_step=step)

                if step % config.test_interval == 0: # 테스트
...
def main():
    ...
    parser.add_argument('--num_test_per_speaker', type=int, default=3)
    parser.add_argument('--random_seed', type=int, default=123)
    parser.add_argument('--summary_interval', type=int, default=100)
    parser.add_argument('--test_interval', type=int, default=5000)
    parser.add_argument('--checkpoint_interval', type=int, default=5000) #checkpoint_interval : 이 간격마다 ckpt에 저장되는 거를 의미 --> max()에러 나면 이거 값을 작게 해주면 돌긴 함,,,
    ...
    # training datasets 분할
    config.data_paths = config.data_paths.split(",")
    # number of multi speaker 
    # setattr / 속성 바꾸기 / 직접 hparams.py에서 수정하지 않아도 됌

    setattr(hparams, "num_speakers", len(config.data_paths)) #setattr(object, name, value): object에 존재하는 속성의 값을 바꾸거나, 새로운 속성을 생성하여 값을 부여


    # model name & log folder 생성 
