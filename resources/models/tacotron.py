...
class Tacotron():
    ...
    def initialize(
        ...
        with tf.variable_scope('inference') as scope:
            ...
            ##############
            # Encoder (character embedding // word to vec)
            # prenet -> cbhg
            ##############

            # [N, T_in, enc_prenet_sizes[-1]]
            # text -> tokenized -> embedding
            # char_embdded_inputs : 'ㄱ'이 임의로 초기화된 값
            # prenet_outputs -> hp.embeddingsize -> enc_prenet_size 
        ...
        # cbhg = 1-D convolution bank + highway network + bidirectional GRU
            ...
            ##############
            # Attention / encoder output 으로 attention진행
            ##############
            ...
            # decoder output 으론 mel spectrum이 나온다.
            # [N, T_out, M]
